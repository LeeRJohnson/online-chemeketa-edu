...
var colui = {
    ...
    scripts: { scrollTo: '/online/scripts/jquery.scrollTo-1.4.2-min.js',
        forms: '/online/scripts/jquery.form.js',
        quicksearch: '/online/scripts/jquery.quicksearch.js'
    },
    load: function(name, version) { $.getScript(colui.scripts.name); },
    header: 'div.hd',
    ...
    navbarLoad: function(selector) {
        ...
        $(selector || '#navbar>.linkbar>li').not('.dir')
            ...
            .one("mouseover", function(i) {
                ...
                if (!$(this).data('src')) {
                    $(this).data('src', $(colui.templates.jLoading).addClass('span-24'));
...
jQuery.fn.navbarButton = function() {
    return this.each(function(i) {
        $(this)
            .data('src').data('nav', $(this))
            .load($(this).attr("href") + ' #content>div.bd', '', function(txt, status, xhr) {
                if ($(this).html() != '') {
                    $(this).addClass('navpane')
                            .data('nav')
